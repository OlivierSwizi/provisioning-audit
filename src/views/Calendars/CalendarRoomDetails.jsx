...
const CalendarDetails = () => {
  ...
  const { selectedRoom, calendarMode, selectedDay, startDay, roomEvents } = useSelector(
  ...
  const setCalendarDate = () => {
    ...
    if (roomRef) workDispatch(selectRoom(roomRef));
  };
  ...
  useEffect(() => {
    if (calendarRef.current && roomEvents) {
      const lEvents = roomEvents.map((event) => ({
        id: event.id,
  ...
  const handleOpenEvent = async (args) => {
    const event = roomEvents.find((e) => e.id === args.e.data.id);
    const updatedEvent = await askUpdateEvent(event, { knownLocation: selectedRoom });
