...
import { Card, Col, Row, message, Typography, Input, Button, Select, Switch } from "antd";
...
const SwiziGroupCreation = () => {
  ...
  const [debouncedGroupDisplayName] = useDebounce(groupDisplayName, 500);
  ...
  useEffect(() => {
    if (!debouncedGroupDisplayName) return setAlreadyExists(false);
    checkGroupDisplayName();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [debouncedGroupDisplayName]);

  const checkGroupDisplayName = async () => {
    const existing = await workDispatch(
      searchGroups(debouncedGroupDisplayName, undefined, true, 1, 1, false),
    );
    setAlreadyExists(
      existing.items.length === 1 && existing.items[0]?.label === debouncedGroupDisplayName,
    );
  };
  ...
  const handleCreateGroup = async () => {
    try {
      const newGroupId = await workDispatch(
        createGroup({
          type: "SWIZI",
          displayName: debouncedGroupDisplayName,
    ...
    <Card bordered={false}>
      <>
            ...
            <Col span={8}>
              <div className="toggle-switch">
                <Switch
                  ...
                  size="small"
                  checked={createIMChannel}
                  onChange={() => setCreateIMChannel(!createIMChannel)}
                />
              </div>
            </Col>
          </Col>
        ...
        {/*createIMChannel && (
          <Row style={{ width: "100%", marginTop: "15px" }}>
            <Col span={8}>
              <Text>{t("im-channel-welcome-message")}</Text>
            </Col>
            <Col span={8}>
              <Input.TextArea
                style={{ width: "100%" }}
                ...
                value={imChannelWelcomeMessage}
                placeholder={t("im-channel-welcome-message-placeholder")}
                onChange={(e) => setIMChannelWelcomeMessage(e.target.value)}
              />
              </Col>}
          </Row>
        )*/}
...
export default SwiziGroupCreation;
